...
export class ReconstructedCurve {
  ...
  calc_control_points() {
    ...
    for (let i = 0; i < this.recon_bezy_curve.height_points.length - 1; i += 3) {
      ...
      let pos = tan1.clone().multiplyScalar(p2.x - p1.x).add(new THREE.Vector3(0, p2.y, 0)).add(gp1);
      ...
      let pos2 = tan4.clone().multiplyScalar(p3.x - p4.x).add(new THREE.Vector3(0, p3.y, 0)).add(gp4);
